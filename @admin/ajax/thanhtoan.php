<?php
	...
	include_once "../phpMailer/class.phpmailer.php";	
		...
		$body .= '
				<tr>
					...
					<table width="100%">
					<tr>
					<td><font size="4">Thông tin đặt hàng từ website <a href="https://www.'.$config_url.'">www.'.$config_url.'</a></font> 
					</td>
					<td align="right";><img src="https://www.'.$config_url.'/'._upload_hinhanh_l.$banner_top[0]['photo'].'" ></td>
					</tr>
					</table>
		...
		<table border="1" bordercolor="#0099CC" align="center" cellpadding="5px" cellspacing="5px" width="100%">';
			if(is_array($_SESSION['cart']))
			{
				...
				for($i=0;$i<$max;$i++){
					...
					if($q==0) continue;
            		$body.='<tr><td>'.($i+1).'</td>';
					$body.='<td> <a href="https://'.$config_url.'/san-pham/'.$pid.'/'.changeTitle($pname).'.html" target="_blank"><img src="https://'.$config_url.'/upload/product/'.get_thumb($pid).'" width="70" /></a></td>';
            		$body.='<td><a href="https://'.$config_url.'/san-pham/'.$pid.'/'.changeTitle($pname).'.html" target="_blank">'.$pname.'</a></td>';
  ...
  <tr>
			  ...
			  <p>Website : <a href="https://'.$config_url.'/">'.$row_setting['ten_'.$lang].'</a></p>
              <td> <tr>'; 
			...
			if($mail->Send()){
				...
				echo  $form_tt = '<input type="hidden" name="virtualPaymentClientURL" id="virtualPaymentClientURL" value="https://mtf.onepay.vn/onecomm-pay/vpc.op">  
				                 ...
				                 <input type="hidden" name="vpc_ReturnURL" id="vpc_ReturnURL" value="https://'.$config_url.'/noidia_php/dr.php" />
