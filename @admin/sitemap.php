<?php	
	...
	function CreateXML($tbl='',$com='',$cap=''){
		...
		if($cap=='1')
		{
			while ($row = mysql_fetch_array($result)) {
				...
				$url->appendChild($domtree->createElement('loc','https://'.$config_url.'/'.$com.'/'.$row['tenkhongdau'].'-'.$row['id']));
			}
		}
		if($cap=='2')
		{
		...
		if($cap=='3')
		{
			while ($row = mysql_fetch_array($result)) {
				...
				$url = $xmlRoot->appendChild($url);
				/* you should enclose the following two lines in a cicle */
				$url->appendChild($domtree->createElement('loc','https://'.$config_url.'/'.$com.'/'.$row['tenkhongdau'].'-'.$row['id'].'.htm'));
			}
		}
		else
		{
			while ($row = mysql_fetch_array($result)) {
				...
				$url = $xmlRoot->appendChild($url);
				/* you should enclose the following two lines in a cicle */
				$url->appendChild($domtree->createElement('loc','https://'.$config_url.'/'.$com.'/'.$row['tenkhongdau'].'-'.$row['id'].'.html'));
			}
		}
		return $url;
	}
	...
	$url = $xmlRoot->appendChild($url);
	/* you should enclose the following two lines in a cicle */
	$url->appendChild($domtree->createElement('loc','https://'.$config_url.'/gioi-thieu.html'));
	
	$url = $domtree->createElement("url");
	$url = $xmlRoot->appendChild($url);
	/* you should enclose the following two lines in a cicle */
	$url->appendChild($domtree->createElement('loc','https://'.$config_url.'/lien-he.html'));
	
	$url = $domtree->createElement("url");
	$url = $xmlRoot->appendChild($url);
	/* you should enclose the following two lines in a cicle */
	$url->appendChild($domtree->createElement('loc','https://'.$config_url.'/dang-ky.html'));
	
	$url = $domtree->createElement("url");
	$url = $xmlRoot->appendChild($url);
	/* you should enclose the following two lines in a cicle */
	$url->appendChild($domtree->createElement('loc','https://'.$config_url.'/dang-nhap.html'));
	
	$url = $domtree->createElement("url");
	$url = $xmlRoot->appendChild($url);
	/* you should enclose the following two lines in a cicle */
	$url->appendChild($domtree->createElement('loc','https://'.$config_url.'/gio-hang.html'));
	
	$url = $domtree->createElement("url");
	$url = $xmlRoot->appendChild($url);
	/* you should enclose the following two lines in a cicle */
	$url->appendChild($domtree->createElement('loc','https://'.$config_url.'/thanh-toan.html'));
